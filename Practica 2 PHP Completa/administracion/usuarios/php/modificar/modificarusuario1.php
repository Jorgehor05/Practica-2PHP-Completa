...
<head>
	<meta charset="UTF-8">
	<title> actualizar la base de datos usuario </title>
    <style>
        body {
...
<!-- Menú de navegación -->
    ...
    <form action="modificarusuario2.php" method="post">
		<h1>Seleccionar Usuario para Modificar</h1>
        ...
        $result = $conn->query("SELECT id_usuario, nombre, apellidos FROM usuarios");
        while ($row = $result->fetch_assoc()) {
			echo "<option value='{$row['id_usuario']}'>{$row['nombre']} {$row['apellidos']}</option>";
        }
        ?>
        </select>
